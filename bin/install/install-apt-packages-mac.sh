...
sudo brew install -y apt-transport-https ca-certificates curl gnupg-agent software-properties-common
...
sudo brew install -y gcc build-essential wget ca-certificates xz-utils fontconfig wkhtmltopdf gdebi

# Libraries
sudo brew install -y libzip-dev libevent-dev libpng-dev libjpeg-dev libxslt-dev libxml2-dev libfreetype6

# Dependencies for pyldap
sudo brew install -y libsasl2-dev libldap2-dev libssl-dev


echo -e "\n--- Installing Python 3 + pip3 --"
sudo brew install -y python3.7 python3-pip python3.7-dev python3.7-venv python3-wheel python3-setuptools virtualenv

echo -e "\n--- Installing Postgres Client & Tools --"
sudo brew install -y libpq-dev postgresql-client-common postgresql-client-12 postgresql-12

echo -e "\n---- Installing Django---"
python3 -m venv $ISP_ROOT/venv
source $ISP_ROOT/venv/bin/activate
sudo brew install build-dep python-psycopg2
pip install wheel 
